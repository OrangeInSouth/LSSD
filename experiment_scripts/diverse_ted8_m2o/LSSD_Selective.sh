...
SAVE_DIR=${project_path}/checkpoints/diverse_ted8_m2o/LSSD_Selective
mkdir -vp $SAVE_DIR
...
${python_path} ${project_path}/fairseq_cli/train.py $path_2_data \
  ...
  --online-distillation-weight 1.0 \
  --selective-online-distillation "hard" \
  --fp16 > ${SAVE_DIR}/train.log 2>&1

bash ${project_path}/experiment_scripts/diverse_ted8_m2o/test.sh ${SAVE_DIR}
